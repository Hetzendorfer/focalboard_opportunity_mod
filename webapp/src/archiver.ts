...
class Archiver {
    ...
    private static async importOpportunityFromFile(boardId: string, file: File): Promise<void> {
        const response = await mutator.importOpportunity(boardId, file)
        if (response.status !== 200) {
            Utils.log('ERROR importing archive: ' + response.text())
        }
    }
    ...
    static importOpportunity(boardId: string, onComplete?: () => void): void {
        const input = document.createElement('input')
        input.type = 'file'
        input.accept = '.xlsx'
        input.onchange = async () => {
            ...
            if (file) {
                ...
                await Archiver.importOpportunityFromFile(boardId, file)
            }
